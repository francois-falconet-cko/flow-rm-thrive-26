 ...
 *
 * The new-shopper address combines three parts, because a collision would
 * silently turn a "new shopper" demo into a returning one:
 * - a timestamp and a random suffix, to separate Lambda instances (each has
 *   its own module state, so a counter alone would repeat across them);
 * - a process counter, to separate calls landing in the same millisecond,
 *   which the timestamp and random suffix alone do not reliably do.
 */
...
function newShopperEmail() {
  ...
  const random = Math.floor(Math.random() * 10000)
    .toString()
    .padStart(4, "0");
  return `newuser+${Date.now()}${random}${newShopperCount}@checkout.com`;
}
