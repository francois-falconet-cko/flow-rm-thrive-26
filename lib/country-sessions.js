/**
 * Per-country Payment Session overrides for Checkout.com Flow.
 * Add a new entry here when wiring another flag button.
 *
 * Only fields that differ from the shared base need to be listed.
 * Nested objects are deep-merged on top of the base payload.
 *
 * @see https://api-reference.checkout.com/tag/Flow#operation/CreatePaymentSession
 */

const BASE_AMOUNT = 6540;

const BASE_PAYMENT_SESSION = {
  amount: BASE_AMOUNT,
  currency: "USD",
  payment_type: "Regular",
  reference: "ORD-THRIVE-DEMO",
  description: "Flow demo payment",
  billing_descriptor: {
    name: "Checkout Demo",
    city: "London",
  },
  customer: {
    email: "demo.shopper@example.com",
    name: "Demo Shopper",
  },
  shipping: {
    address: {
      address_line1: "123 Market Street",
      address_line2: "Suite 100",
      city: "San Francisco",
      state: "CA",
      zip: "94105",
      country: "US",
    },
    phone: {
      number: "4155552671",
      country_code: "+1",
    },
  },
  billing: {
    address: {
      address_line1: "123 Market Street",
      address_line2: "Suite 100",
      city: "San Francisco",
      state: "CA",
      zip: "94105",
      country: "US",
    },
    phone: {
      number: "4155552671",
      country_code: "+1",
    },
  },
  risk: {
    enabled: true,
  },
  metadata: {
    demo: "thrive-flow",
  },
  processing_channel_id: "pc_7tqpcv74s6wupapsfzo3fsgutu",
  items: [
    {
      name: "Demo product",
      quantity: 1,
      unit_price: BASE_AMOUNT,
      total_amount: BASE_AMOUNT,
    },
  ],
};

/**
 * Country-specific overrides keyed by lowercase ISO 3166-1 alpha-2 code.
 * Start with US; extend this map country by country.
 */
const COUNTRY_SESSION_OVERRIDES = {
  us: {
    currency: "USD",
    payment_type: "Regular",
    processing_channel_id: "pc_7tqpcv74s6wupapsfzo3fsgutu",
    customer: {
      email: "us.shopper@example.com",
      name: "Alex Morgan",
    },
    billing: {
      address: {
        address_line1: "123 Market Street",
        address_line2: "Suite 100",
        city: "San Francisco",
        state: "CA",
        zip: "94105",
        country: "US",
      },
      phone: {
        number: "4155552671",
        country_code: "+1",
      },
    },
    shipping: {
      address: {
        address_line1: "123 Market Street",
        address_line2: "Suite 100",
        city: "San Francisco",
        state: "CA",
        zip: "94105",
        country: "US",
      },
      phone: {
        number: "4155552671",
        country_code: "+1",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  ca: {
    currency: "CAD",
    payment_type: "Regular",
    processing_channel_id: "pc_jx7ges3jql2ufdp4iqxkicnymu",
    customer: {
      email: "ca.shopper@example.com",
      name: "Jordan Lee",
    },
    billing: {
      address: {
        address_line1: "100 Queen Street West",
        address_line2: "Floor 5",
        city: "Toronto",
        state: "ON",
        zip: "M5H 2N2",
        country: "CA",
      },
      phone: {
        number: "4165550199",
        country_code: "+1",
      },
    },
    shipping: {
      address: {
        address_line1: "100 Queen Street West",
        address_line2: "Floor 5",
        city: "Toronto",
        state: "ON",
        zip: "M5H 2N2",
        country: "CA",
      },
      phone: {
        number: "4165550199",
        country_code: "+1",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  br: {
    currency: "BRL",
    payment_type: "Regular",
    processing_channel_id: "pc_cmen3vj7i6wevor7u5kmgcvytm",
    customer: {
      email: "br.shopper@example.com",
      name: "Ana Silva",
    },
    billing: {
      address: {
        address_line1: "Avenida Paulista 1000",
        address_line2: "Sala 12",
        city: "Sao Paulo",
        state: "SP",
        zip: "01310-100",
        country: "BR",
      },
      phone: {
        number: "11987654321",
        country_code: "+55",
      },
    },
    shipping: {
      address: {
        address_line1: "Avenida Paulista 1000",
        address_line2: "Sala 12",
        city: "Sao Paulo",
        state: "SP",
        zip: "01310-100",
        country: "BR",
      },
      phone: {
        number: "11987654321",
        country_code: "+55",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  ae: {
    currency: "AED",
    payment_type: "Regular",
    processing_channel_id: "pc_vuvz273nbk5efdstxob2mk3yse",
    customer: {
      email: "ae.shopper@example.com",
      name: "Omar Al Farsi",
      summary: {
        registration_date: "2023-05-01",
        first_transaction_date: "2023-07-01",
        last_payment_date: "2023-08-01",
        total_order_count: 15,
        last_payment_amount: 500,
        is_premium_customer: true,
        is_returning_customer: true,
        lifetime_value: 5000,
      },
    },
    billing: {
      address: {
        address_line1: "Olaya Street, Building 12",
        city: "Riyadh",
        zip: "11432",
        country: "AE",
      },
      phone: {
        number: "501234567",
        country_code: "+966",
      },
    },
    shipping: {
      address: {
        address_line1: "Ulaya Dist., P.O.Box: 5665",
        city: "Riyadh",
        zip: "11432",
        country: "AE",
      },
      phone: {
        number: "501234567",
        country_code: "+966",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  sa: {
    currency: "SAR",
    payment_type: "Regular",
    processing_channel_id: "pc_vuvz273nbk5efdstxob2mk3yse",
    customer: {
      email: "sa.shopper@example.com",
      name: "Omar Al Farsi",
      summary: {
        registration_date: "2023-05-01",
        first_transaction_date: "2023-07-01",
        last_payment_date: "2023-08-01",
        total_order_count: 15,
        last_payment_amount: 500,
        is_premium_customer: true,
        is_returning_customer: true,
        lifetime_value: 5000,
      },
    },
    billing: {
      address: {
        address_line1: "Olaya Street, Building 12",
        city: "Riyadh",
        zip: "11432",
        country: "SA",
      },
      phone: {
        number: "501234567",
        country_code: "+966",
      },
    },
    shipping: {
      address: {
        address_line1: "Ulaya Dist., P.O.Box: 5665",
        city: "Riyadh",
        zip: "11432",
        country: "SA",
      },
      phone: {
        number: "501234567",
        country_code: "+966",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  gb: {
    currency: "GBP",
    payment_type: "Regular",
    processing_channel_id: "pc_mfvsc6aeff4elmgartdk3ovn2i",
    customer: {
      email: "gb.shopper@example.com",
      name: "Jamie Cole",
    },
    billing: {
      address: {
        address_line1: "123 High Street",
        city: "London",
        zip: "SW1A 1AA",
        country: "GB",
      },
      phone: {
        number: "2079460958",
        country_code: "+44",
      },
    },
    shipping: {
      address: {
        address_line1: "123 High Street",
        city: "London",
        zip: "SW1A 1AA",
        country: "GB",
      },
      phone: {
        number: "2079460958",
        country_code: "+44",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  nl: {
    currency: "EUR",
    payment_type: "Regular",
    processing_channel_id: "pc_anuu7qlqknnujm3olh3ajsqs5q",
    customer: {
      email: "nl.shopper@example.com",
      name: "Sanne de Vries",
    },
    billing: {
      address: {
        address_line1: "Damrak 70",
        city: "Amsterdam",
        zip: "1012 LM",
        country: "NL",
      },
      phone: {
        number: "612345678",
        country_code: "+31",
      },
    },
    shipping: {
      address: {
        address_line1: "Damrak 70",
        city: "Amsterdam",
        zip: "1012 LM",
        country: "NL",
      },
      phone: {
        number: "612345678",
        country_code: "+31",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  de: {
    currency: "EUR",
    payment_type: "Regular",
    processing_channel_id: "pc_anuu7qlqknnujm3olh3ajsqs5q",
    customer: {
      email: "de.shopper@example.com",
      name: "Lena Müller",
    },
    billing: {
      address: {
        address_line1: "Friedrichstrasse 100",
        city: "Berlin",
        zip: "10117",
        country: "DE",
      },
      phone: {
        number: "3012345678",
        country_code: "+49",
      },
    },
    shipping: {
      address: {
        address_line1: "Friedrichstrasse 100",
        city: "Berlin",
        zip: "10117",
        country: "DE",
      },
      phone: {
        number: "3012345678",
        country_code: "+49",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  fr: {
    currency: "EUR",
    payment_type: "Regular",
    processing_channel_id: "pc_anuu7qlqknnujm3olh3ajsqs5q",
    customer: {
      email: "fr.shopper@example.com",
      name: "Camille Durand",
    },
    billing: {
      address: {
        address_line1: "12 Rue de Rivoli",
        city: "Paris",
        zip: "75004",
        country: "FR",
      },
      phone: {
        number: "612345678",
        country_code: "+33",
      },
    },
    shipping: {
      address: {
        address_line1: "12 Rue de Rivoli",
        city: "Paris",
        zip: "75004",
        country: "FR",
      },
      phone: {
        number: "612345678",
        country_code: "+33",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  sg: {
    currency: "SGD",
    payment_type: "Regular",
    processing_channel_id: "pc_njuuky6x3ekepcmfe7esadbipu",
    customer: {
      email: "sg.shopper@example.com",
      name: "Wei Ling Tan",
    },
    billing: {
      address: {
        address_line1: "1 Raffles Place",
        address_line2: "Tower 2, Level 15",
        city: "Singapore",
        zip: "048616",
        country: "SG",
      },
      phone: {
        number: "81234567",
        country_code: "+65",
      },
    },
    shipping: {
      address: {
        address_line1: "1 Raffles Place",
        address_line2: "Tower 2, Level 15",
        city: "Singapore",
        zip: "048616",
        country: "SG",
      },
      phone: {
        number: "81234567",
        country_code: "+65",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
      },
    ],
  },

  hk: {
    currency: "HKD",
    payment_type: "Regular",
    processing_channel_id: "pc_intrqwc5tysuhim7xfd62wgmfm",
    customer: {
      email: "hk.shopper@example.com",
      name: "Wai Man Chan",
    },
    billing: {
      address: {
        address_line1: "8 Finance Street",
        address_line2: "Level 20",
        city: "Central",
        zip: "999077",
        country: "HK",
      },
      phone: {
        number: "51234567",
        country_code: "+852",
      },
    },
    shipping: {
      address: {
        address_line1: "8 Finance Street",
        address_line2: "Level 20",
        city: "Central",
        zip: "999077",
        country: "HK",
      },
      phone: {
        number: "51234567",
        country_code: "+852",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        reference: "demo-product-hk",
        unit_price: BASE_AMOUNT,
      },
    ],
  },

  qa: {
    currency: "QAR",
    payment_type: "Regular",
    processing_channel_id: "pc_anuu7qlqknnujm3olh3ajsqs5q",
    customer: {
      email: "qa.shopper@example.com",
      name: "Fatima Al Thani",
    },
    billing: {
      address: {
        address_line1: "Al Corniche Street 18",
        city: "Doha",
        zip: "00000",
        country: "QA",
      },
      phone: {
        number: "33123456",
        country_code: "+974",
      },
    },
    shipping: {
      address: {
        address_line1: "Al Corniche Street 18",
        city: "Doha",
        zip: "00000",
        country: "QA",
      },
      phone: {
        number: "33123456",
        country_code: "+974",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  bh: {
    currency: "BHD",
    payment_type: "Regular",
    processing_channel_id: "pc_anuu7qlqknnujm3olh3ajsqs5q",
    customer: {
      email: "bh.shopper@example.com",
      name: "Yusuf Al Khalifa",
    },
    billing: {
      address: {
        address_line1: "Road 1705, Building 247",
        city: "Manama",
        zip: "317",
        country: "BH",
      },
      phone: {
        number: "36001234",
        country_code: "+973",
      },
    },
    reference: "ORD-THRIVE-DEMO-BH",
    shipping: {
      address: {
        address_line1: "Road 1705, Building 247",
        city: "Manama",
        zip: "317",
        country: "BH",
      },
      phone: {
        number: "36001234",
        country_code: "+973",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  au: {
    currency: "AUD",
    payment_type: "Regular",
    processing_channel_id: "pc_w2we6tymw7qelgiybpeifllrz4",
    customer: {
      email: "au.shopper@example.com",
      name: "Olivia Bennett",
    },
    billing: {
      address: {
        address_line1: "200 George Street",
        address_line2: "Level 12",
        city: "Sydney",
        state: "NSW",
        zip: "2000",
        country: "AU",
      },
      phone: {
        number: "412345678",
        country_code: "+61",
      },
    },
    shipping: {
      address: {
        address_line1: "200 George Street",
        address_line2: "Level 12",
        city: "Sydney",
        state: "NSW",
        zip: "2000",
        country: "AU",
      },
      phone: {
        number: "412345678",
        country_code: "+61",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  jp: {
    currency: "JPY",
    payment_type: "Regular",
    processing_channel_id: "pc_n32lfgr6lbnutkvoua5xc4ozxe",
    customer: {
      email: "jp.shopper@example.com",
      name: "Haruto Sato",
    },
    billing: {
      address: {
        address_line1: "2-1-1 Nihonbashi",
        address_line2: "Chuo-ku",
        city: "Tokyo",
        zip: "103-0027",
        country: "JP",
      },
      phone: {
        number: "9012345678",
        country_code: "+81",
      },
    },
    shipping: {
      address: {
        address_line1: "2-1-1 Nihonbashi",
        address_line2: "Chuo-ku",
        city: "Tokyo",
        zip: "103-0027",
        country: "JP",
      },
      phone: {
        number: "9012345678",
        country_code: "+81",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  se: {
    currency: "SEK",
    payment_type: "Regular",
    processing_channel_id: "pc_anuu7qlqknnujm3olh3ajsqs5q",
    customer: {
      email: "se.shopper@example.com",
      name: "Elsa Lindqvist",
    },
    billing: {
      address: {
        address_line1: "Drottninggatan 30",
        city: "Stockholm",
        zip: "111 51",
        country: "SE",
      },
      phone: {
        number: "701234567",
        country_code: "+46",
      },
    },
    shipping: {
      address: {
        address_line1: "Drottninggatan 30",
        city: "Stockholm",
        zip: "111 51",
        country: "SE",
      },
      phone: {
        number: "701234567",
        country_code: "+46",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  be: {
    currency: "EUR",
    payment_type: "Regular",
    processing_channel_id: "pc_anuu7qlqknnujm3olh3ajsqs5q",
    customer: {
      email: "be.shopper@example.com",
      name: "Louise Dubois",
    },
    billing: {
      address: {
        address_line1: "Rue Neuve 42",
        city: "Brussels",
        zip: "1000",
        country: "BE",
      },
      phone: {
        number: "470123456",
        country_code: "+32",
      },
    },
    shipping: {
      address: {
        address_line1: "Rue Neuve 42",
        city: "Brussels",
        zip: "1000",
        country: "BE",
      },
      phone: {
        number: "470123456",
        country_code: "+32",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  dk: {
    currency: "DKK",
    payment_type: "Regular",
    processing_channel_id: "pc_anuu7qlqknnujm3olh3ajsqs5q",
    customer: {
      email: "dk.shopper@example.com",
      name: "Mads Nielsen",
    },
    billing: {
      address: {
        address_line1: "Vesterbrogade 12",
        city: "Copenhagen",
        zip: "1620",
        country: "DK",
      },
      phone: {
        number: "20123456",
        country_code: "+45",
      },
    },
    shipping: {
      address: {
        address_line1: "Vesterbrogade 12",
        city: "Copenhagen",
        zip: "1620",
        country: "DK",
      },
      phone: {
        number: "20123456",
        country_code: "+45",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  no: {
    currency: "NOK",
    payment_type: "Regular",
    processing_channel_id: "pc_anuu7qlqknnujm3olh3ajsqs5q",
    customer: {
      email: "no.shopper@example.com",
      name: "Ingrid Hansen",
    },
    billing: {
      address: {
        address_line1: "Karl Johans gate 22",
        city: "Oslo",
        zip: "0159",
        country: "NO",
      },
      phone: {
        number: "40612345",
        country_code: "+47",
      },
    },
    shipping: {
      address: {
        address_line1: "Karl Johans gate 22",
        city: "Oslo",
        zip: "0159",
        country: "NO",
      },
      phone: {
        number: "40612345",
        country_code: "+47",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  es: {
    currency: "EUR",
    payment_type: "Regular",
    processing_channel_id: "pc_anuu7qlqknnujm3olh3ajsqs5q",
    customer: {
      email: "es.shopper@example.com",
      name: "Lucia Garcia",
    },
    billing: {
      address: {
        address_line1: "Calle Gran Via 28",
        city: "Madrid",
        zip: "28013",
        country: "ES",
      },
      phone: {
        number: "612345678",
        country_code: "+34",
      },
    },
    shipping: {
      address: {
        address_line1: "Calle Gran Via 28",
        city: "Madrid",
        zip: "28013",
        country: "ES",
      },
      phone: {
        number: "612345678",
        country_code: "+34",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },

  pt: {
    currency: "EUR",
    payment_type: "Regular",
    processing_channel_id: "pc_anuu7qlqknnujm3olh3ajsqs5q",
    customer: {
      email: "pt.shopper@example.com",
      name: "Miguel Santos",
    },
    billing: {
      address: {
        address_line1: "Avenida da Liberdade 100",
        city: "Lisbon",
        zip: "1250-096",
        country: "PT",
      },
      phone: {
        number: "912345678",
        country_code: "+351",
      },
    },
    shipping: {
      address: {
        address_line1: "Avenida da Liberdade 100",
        city: "Lisbon",
        zip: "1250-096",
        country: "PT",
      },
      phone: {
        number: "912345678",
        country_code: "+351",
      },
    },
    items: [
      {
        name: "Demo product",
        quantity: 1,
        unit_price: BASE_AMOUNT,
        total_amount: BASE_AMOUNT,
      },
    ],
  },
};

function isPlainObject(value) {
  return value !== null && typeof value === "object" && !Array.isArray(value);
}

function deepMerge(base, override) {
  if (!isPlainObject(base) || !isPlainObject(override)) {
    return override === undefined ? base : override;
  }

  const result = { ...base };

  for (const [key, value] of Object.entries(override)) {
    if (Array.isArray(value)) {
      result[key] = value.slice();
    } else if (isPlainObject(value) && isPlainObject(base[key])) {
      result[key] = deepMerge(base[key], value);
    } else {
      result[key] = value;
    }
  }

  return result;
}

/**
 * Alternative processing channels a session can be created on, on top of the
 * country config. Used by the Remember Me demos: same US session, different
 * channel per presentation mode.
 */
const PROCESSING_CHANNEL_VARIANTS = {
  "rm-checkbox": "pc_g27bmypp5ryetnywyfdyzyczkm",
  "rm-embedded": "pc_nkzprmxd5r6uvhklbuq5xdnsh4",
};

function normalizeVariant(variant) {
  return String(variant || "")
    .trim()
    .toLowerCase();
}

function isVariantSupported(variant) {
  const key = normalizeVariant(variant);
  return key === "" || Boolean(PROCESSING_CHANNEL_VARIANTS[key]);
}

function getSupportedVariants() {
  return Object.keys(PROCESSING_CHANNEL_VARIANTS);
}

/**
 * Remember Me shopper journeys. The journey is expressed entirely through
 * `customer.email`, since that is what Remember Me recognises a shopper by:
 *
 * - "new"       → a fresh address on every session, so Remember Me has never
 *                 seen it and Flow shows the save-card path.
 * - "returning" → one fixed address with cards already enrolled.
 *
 * The new-shopper suffix is kept to 8 digits so it stays readable on stage.
 * It is the low 8 digits of the millisecond clock, offset by a process
 * counter: the clock makes consecutive demos differ, and the counter covers
 * two calls landing in the same millisecond — a collision would silently turn
 * a "new shopper" demo into a returning one.
 */
const RETURNING_SHOPPER_EMAIL = "returning.user@checkout.com";

const NEW_SHOPPER_DIGITS = 8;

let newShopperCount = 0;

function newShopperEmail() {
  newShopperCount += 1;
  const modulus = 10 ** NEW_SHOPPER_DIGITS;
  const suffix = (Date.now() + newShopperCount) % modulus;
  return `newuser+${String(suffix).padStart(NEW_SHOPPER_DIGITS, "0")}@checkout.com`;
}

const SHOPPER_JOURNEYS = {
  new: newShopperEmail,
  returning: () => RETURNING_SHOPPER_EMAIL,
};

function normalizeJourney(journey) {
  return String(journey || "")
    .trim()
    .toLowerCase();
}

function isJourneySupported(journey) {
  const key = normalizeJourney(journey);
  return key === "" || Boolean(SHOPPER_JOURNEYS[key]);
}

function getSupportedJourneys() {
  return Object.keys(SHOPPER_JOURNEYS);
}

function getSupportedCountryCodes() {
  return Object.keys(COUNTRY_SESSION_OVERRIDES);
}

function isCountrySupported(countryCode) {
  return Boolean(COUNTRY_SESSION_OVERRIDES[normalizeCountryCode(countryCode)]);
}

function normalizeCountryCode(countryCode) {
  return String(countryCode || "")
    .trim()
    .toLowerCase();
}

function getAppBaseUrl() {
  const fromEnv = process.env.APP_BASE_URL || process.env.PUBLIC_URL;
  if (fromEnv) {
    return fromEnv.replace(/\/$/, "");
  }

  const port = process.env.PORT || 3000;
  return `http://localhost:${port}`;
}

/**
 * Build the Create Payment Session request body for a given country flag.
 * @param {string} countryCode - e.g. "us"
 * @param {string} [variant] - processing-channel variant, e.g. "rm-checkbox"
 * @param {string} [journey] - shopper journey, "new" or "returning"
 * @returns {object}
 */
function buildPaymentSessionPayload(countryCode, variant, journey) {
  const code = normalizeCountryCode(countryCode);
  const overrides = COUNTRY_SESSION_OVERRIDES[code];

  if (!overrides) {
    const error = new Error(
      `No payment-session config for country "${countryCode}". ` +
        `Supported: ${getSupportedCountryCodes().join(", ") || "(none)"}`,
    );
    error.statusCode = 400;
    throw error;
  }

  const variantKey = normalizeVariant(variant);
  const variantChannel = PROCESSING_CHANNEL_VARIANTS[variantKey];

  if (variantKey && !variantChannel) {
    const error = new Error(
      `Unknown processing-channel variant "${variant}". ` +
        `Supported: ${getSupportedVariants().join(", ")}`,
    );
    error.statusCode = 400;
    throw error;
  }

  const journeyKey = normalizeJourney(journey);
  const journeyEmail = SHOPPER_JOURNEYS[journeyKey];

  if (journeyKey && !journeyEmail) {
    const error = new Error(
      `Unknown shopper journey "${journey}". ` +
        `Supported: ${getSupportedJourneys().join(", ")}`,
    );
    error.statusCode = 400;
    throw error;
  }

  const baseUrl = getAppBaseUrl();

  return deepMerge(BASE_PAYMENT_SESSION, {
    ...overrides,
    // A variant swaps only the processing channel; the country config stands.
    ...(variantChannel ? { processing_channel_id: variantChannel } : {}),
    // A journey swaps only the shopper email, keeping the country's customer
    // name — so spread the country's customer rather than replacing it.
    ...(journeyEmail
      ? { customer: { ...overrides.customer, email: journeyEmail() } }
      : {}),
    success_url: `${baseUrl}/?status=succeeded`,
    failure_url: `${baseUrl}/?status=failed`,
    metadata: {
      ...BASE_PAYMENT_SESSION.metadata,
      country: code.toUpperCase(),
      ...(variantKey ? { variant: variantKey } : {}),
      ...(journeyKey ? { journey: journeyKey } : {}),
    },
  });
}

module.exports = {
  BASE_AMOUNT,
  BASE_PAYMENT_SESSION,
  COUNTRY_SESSION_OVERRIDES,
  PROCESSING_CHANNEL_VARIANTS,
  RETURNING_SHOPPER_EMAIL,
  SHOPPER_JOURNEYS,
  buildPaymentSessionPayload,
  getSupportedCountryCodes,
  getSupportedJourneys,
  getSupportedVariants,
  isCountrySupported,
  isJourneySupported,
  isVariantSupported,
  normalizeCountryCode,
  normalizeJourney,
  normalizeVariant,
};
